...
void loop() {
  ...
  if (automode == 1) {currentMode[1]++; currentMode[0]++;}
  poolButtons();
...
///////////////////////////////////////////////////////turn columns off for PROPELLER
void turnColumnsOff()
{
  poolButtons();
  for (int i = 0; i < 16; i++)
...
void turnOnAndOffAllByLayerUpAndDownNotTimed()//++++++++++4
{
  int x = 100;
  for (int i = 5; i != 0; i--)
  {
...
void turnOnAndOffAllByColumnSideways()//++++++++++1
{
  int x = 75;
  turnEverythingOff();
  //turn on layers
...
void goThroughAllLedsOneAtATime()//++++++10
{
  int x = 15;
  turnEverythingOff();
  for (int y = 0; y < 5; y++)
